...
def main():
    ...
    versions = ['15.0.0'] # get_unicode_versions()
...
def do_east_asian(versions):
    ...
    for version in versions:
        ...
        url = 'https://github.com/waltarix/localedata/releases/download/{version}/EastAsianWidth.txt'
        try:
...
def do_zero_width(versions):
    ...
    for version in versions:
        ...
        url = 'http://www.unicode.org/Public/{version}/ucd/extracted/DerivedGeneralCategory.txt'
        try:
            do_retrieve(url=url.format(version=version),
                        fname=fin.format(version=version))
        except urllib.error.HTTPError as err:
